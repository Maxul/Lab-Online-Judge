...
int main(int argc, char *argv[])
{
	...
	for ( ; ; ) {
		...
		//printf("%llu is in(%d)?\n", REG_SYS_CALL(&regs), isAllowedCall(REG_SYS_CALL(&regs)));
		if (0 == isAllowedCall(REG_SYS_CALL(&regs))) {
			...
			for (i = 0; i < 100; i++) {
				file_tmp[i] = ptrace(PTRACE_PEEKDATA, child, REG_ARG_1(&regs) + i * sizeof(long), NULL);
				if (file_tmp[i] == 0)
					break;
			}
			...
			//printf("%s:%lld\n", (const char*)file_tmp, REG_ARG_2(&regs));
		}

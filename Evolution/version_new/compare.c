...
int main(int argc, char *argv[], char *env[])
{
	...
	if (3 != argc)
		MSG_ERR_RET(EXIT_FAILURE, "Usage: %s out_file temp_file\n", argv[0]);

	if ((fd[0] = open(out_file, O_RDONLY, 0644)) < 0)
		MSG_ERR_RET(SYSTEM_ERROR, "open(out_file) Failed");

	if ((fd[1] = open(tmp_file, O_RDONLY, 0644)) < 0)
		MSG_ERR_RET(SYSTEM_ERROR, "open(tmp_file) Failed");
	...
	if (-1 == out_len || -1 == tmp_len)
		MSG_ERR_RET(SYSTEM_ERROR, "lseek() Failed");
